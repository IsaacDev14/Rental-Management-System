// frontend/src/pages/management/PaymentManagement.tsx

import React, { useState, useMemo, useContext, useEffect } from 'react';
import { PlusCircle, Edit, Trash2, DollarSign, Receipt, AlertTriangle, Search, ChevronDown } from 'lucide-react'; // Import icons
...
import { Payment, Tenant, Property } from '../../types/models'; // Import Payment, Tenant, Property types

/**
 * PaymentManagement component.
 * Allows landlords to record, view, and manage rent payments.
...
const PaymentManagement: React.FC = () => {
  ...
  // Filter payments to show only those associated with the current landlord's tenants
  ...
  // Filtered payments based on search term and status
  ...
  /**
   * Handles opening the modal to record a new payment.
   */
  ...
  /**
   * Handles opening the modal to edit an existing payment.
  ...
  /**
   * Handles saving a new or updated payment.
   * Updates the global data state and logs the action.
   * @param paymentData The data of the payment to save.
   */
  ...
  /**
   * Handles initiating payment deletion.
   * @param payment The payment object to delete.
   */
  ...
  /**
   * Performs the actual deletion of a payment.
   */
  ...
  /**
   * PaymentForm component (nested for simplicity).
   * Form for recording/editing payment details.
   */
  const PaymentForm: React.FC<{ payment: Payment | null; onSave: (data: Payment) => void; onCancel: () => void; tenants: Tenant[]; properties: Property[] }> = ({ payment, onSave, onCancel, tenants, properties }) => {
    ...
    // Update form data if editingPayment changes
    useEffect(() => {
      if (payment) {
        setFormData(payment);
      } else {
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-6">
        <SelectField
          ...
          options={tenants.map(t => ({ value: t.id, label: `${t.name} (${properties.find(p => p.id === t.propertyId)?.name || 'N/A'})` }))}
          required
        />
        ...
        <SelectField
          ...
          onChange={handleChange}
          error={errors.method}
          options={[{ value: 'M-PESA', label: 'M-PESA' }, { value: 'Bank Transfer', label: 'Bank Transfer' }, { value: 'Cash', label: 'Cash' }]}
          required
        />
        <SelectField
          id="status"
          ...
          onChange={handleChange}
          error={errors.status}
          options={[{ value: 'Paid', label: 'Paid' }, { value: 'Overdue', label: 'Overdue' }, { value: 'Pending', label: 'Pending' }]}
          required
        />
  ...
  const paymentTableColumns = useMemo(() => [
    ...
    { key: 'method', header: 'Method' },
    {
      ...
      header: 'Status',
      render: (row: Payment) => (
        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${row.status === 'Paid' ? 'bg-emerald-500/20 text-emerald-400' : row.status === 'Overdue' ? 'bg-red-500/20 text-red-400' : 'bg-amber-500/20 text-amber-400'}`}>
          {row.status}
        </span>
      ),
    },
    {
      ...
      render: (row: Payment) => (
        <div className="flex space-x-2">
          ...
          {row.status === 'Paid' && (
            <Button variant="ghost" size="sm" onClick={() => { /* Implement view receipt logic here */ }} title="View Receipt">
              <Receipt size={18} />
            </Button>
          )}
        </div>
  ...
  ], [data.tenants]); // Re-memoize if tenants change to update tenant names
    ...
    <>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingPayment ? "Edit Payment" : "Record New Payment"}>
        <PaymentForm
          ...
          tenants={landlordTenants} // Pass only landlord's tenants
          properties={landlordProperties} // Pass landlord's properties for tenant display
        />
      </Modal>

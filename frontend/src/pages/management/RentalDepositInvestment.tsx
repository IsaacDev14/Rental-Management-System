...
import { Shield, TrendingUp, Briefcase, Wallet, AlertTriangle, Search, ChevronDown } from 'lucide-react';
...
import type { Deposit } from '../../types/models'; // type-only imports
...
const RentalDepositInvestment: React.FC = () => {
  ...
  // Uncomment if you need to use landlordProperties later
  ...
  const totalDepositsHeld = landlordDeposits
    .filter(d => d.status === 'Held')
    .reduce((sum, d) => sum + d.amount, 0);

  const simulatedInterestRate = data.investmentFunds.find(f => f.id === 'mmf1')?.currentRate || 0.08;
  ...
  const performRefundDeposit = () => {
    if (depositToRefund) {
      setData(prev => {
        const updatedDeposits: Deposit[] = prev.deposits.map(d =>
    ...
    { key: 'date', header: 'Date Received' },
    {
      ...
      render: (row: Deposit) => (
        <span className={`px-3 py-1 text-xs rounded-full font-semibold ${
          row.status === 'Held' ? 'bg-blue-500/20 text-blue-400' :
          row.status === 'Refunded' ? 'bg-emerald-500/20 text-emerald-400' :
        ...
        <div className="bg-gray-800 p-6 rounded-xl shadow-xl border border-gray-700 text-white">
          <div className="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0">
            <h3 className="text-xl font-semibold">
              Deposit Ledger {isLandlord && `for ${data.landlords.find(l => l.id === currentUserId)?.name || 'N/A'}`}
            </h3>
          ...
          {isLandlord && (
            <div className="mt-6 p-4 bg-gray-700 rounded-lg border border-gray-600">
              ...
              <p className="text-sm text-gray-300 mb-2">
                Current Investment Fund: <span className="font-semibold text-cyan-400">{data.investmentFunds.find(f => f.id === 'mmf1')?.name}</span> (Rate: {(simulatedInterestRate * 100).toFixed(2)}% p.a. - simulated)
              </p>
              <p className="text-sm text-gray-300 mb-2">
                Landlords can opt into interest-earning deposits. Funds are pooled and invested in low-risk Money Market Funds. Interest is calculated monthly and allocated to your portfolio.
              </p>
